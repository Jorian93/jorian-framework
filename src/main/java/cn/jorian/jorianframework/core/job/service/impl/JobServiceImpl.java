...
public class JobServiceImpl extends ServiceImpl<JobMapper, Job> implements JobService {
	...
	public void updateJob(Job job) {
		...
		CronTrigger cronTrigger = TriggerBuilder.newTrigger().withIdentity(jobName).withSchedule(cronScheduleBuilder)
				.forJob(jobKey).build();
		try {
	...
	public void saveJob(JobAddDTO jobAddDTO) {
		...
		if (findJob != null) {
			throw new IllegalArgumentException(jobAddDTO.getJobName() + "已存在");
		}
		findJob = new Job();
